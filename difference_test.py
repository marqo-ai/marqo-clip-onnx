...
def onnx_evaluation(SOURCE, MODEL_NAME, PRETRAINED):
    ...
    for model in model_list:
        ...
        if len(os.listdir(dir)) == 1 and (os.path.basename(model) in os.listdir(dir)):
            MODEL_NAME = os.path.basename(model)

        elif len(os.listdir(dir)) > 1 and (os.path.basename(model) in os.listdir(dir)):
            MODEL_NAME = os.path.basename(model).replace(".onnx", ".zip")
            shutil.make_archive(MODEL_NAME.replace(".zip", ""), "zip", dir)
